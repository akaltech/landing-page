...
- `npx shadcn@latest add <component-name>` — Add a shadcn/ui component
...
Components are copied into `components/ui/` and can be freely modified. CSS variables are defined in `app/globals.css`. The button uses `rounded-full` to match our minimalist aesthetic.
